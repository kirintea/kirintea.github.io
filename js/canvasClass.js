...
class DameDaneParticle {
  ...
  constructor(canvas, options, callback) {
    ...
    // 保存原始spacing值
    this._originalSpacing = options.spacing;
    ...
    this.IMG.onload = function() {
      ...
      eleCtx.clearRect(0, 0, this.canvasEle.width, this.canvasEle.height);
      // 第一次初始化图片
  ...
  /** 更新canvas大小为容器大小 */
  updateCanvasSize() {
    const container = this.canvasEle.parentElement;
    if (container) {
      this.canvasEle.width = container.clientWidth;
      this.canvasEle.height = container.clientHeight;
    } else {
  ...
  updateRenderPosition() {
    ...
    // 使用原始spacing计算图片渲染后的尺寸
    let renderedWidth = this.ImgW * this._originalSpacing;
    let renderedHeight = this.ImgH * this._originalSpacing;
    
    // 预留边距（顶部和底部各预留2.5%的空间）
    const padding = containerHeight * 0.025;
    const availableHeight = containerHeight - padding * 2;
    
    // 计算缩放比例，让图片填满可用空间
    let scale = availableHeight / renderedHeight;
    
    // 应用缩放后的spacing
    this.options.spacing = this._originalSpacing * scale;
    
    // 重新计算渲染尺寸
    renderedWidth = this.ImgW * this.options.spacing;
    renderedHeight = this.ImgH * this.options.spacing;
    
    // 计算居中位置
    this.renderX = (containerWidth - renderedWidth) / 2;
    this.renderY = (containerHeight - renderedHeight) / 2;
  }
   ...
   */
  _InitParticle(ImgData, rebuildParticle) {
    if (rebuildParticle === undefined) rebuildParticle = false;
  ...
  _Draw2Canvas() {
    ...
    this.PointArr.forEach(
      function(point) {
        point.update(self.ParticlePolymerizeFlag, self.options, self.mx, self.my);
        point.render();
      }
    );
